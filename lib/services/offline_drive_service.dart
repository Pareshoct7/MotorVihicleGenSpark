...
class OfflineDriveService {
  ...
  static Future<String?> saveInspectionPdf(Inspection inspection, Uint8List pdfBytes) async {
    if (_rootDir == null) await init();
    ...
    try {
      ...
      final month = DateFormat('MMMM').format(inspection.inspectionDate); // Full month name
  ...
  static Future<void> generateAllBackfill({
    ...
    for (final vehicle in vehicles) {
      final store = stores.firstWhere(
        ...
        orElse: () => defaultStore ?? Store(id: 'unknown', name: 'Unknown Store', address: ''),
      );
      
      // Try to find a driver. Vehicles don't have driverId, so use default.
      final driver = drivers.isNotEmpty 
          ? drivers.first
          : Driver(id: 'unknown', name: 'Unknown Driver', licenseNumber: '', phone: '');
      ...
      for (final date in odometerMap.keys) {
        ...
        bool existsInHive = DatabaseService.hasInspectionInSameWeek(vehicle.id, date);
        
        Inspection? inspection;

        if (!existsInHive) {
            // Generate NEW Inspection
            onProgress(currentStep, totalSteps, 'Generating missing report for ${vehicle.registrationNo} - ${DateFormat('dd/MM').format(date)}');
            ...
            await DatabaseService.addInspection(inspection);
             ...
             try {
               ...
               inspection = all.firstWhere((i) => i.vehicleId == vehicle.id && _isSameWeek(i.inspectionDate, date));
             } catch (e) {
        ...
        // 2. Check/Save PDF
        if (inspection != null) {
          onProgress(currentStep, totalSteps, 'Verifying PDF for ${vehicle.registrationNo}...');
          ...
          if (!await file.exists()) {
             try {
               final pdfBytes = await PdfService.generateTemplateMatchingPdf(inspection);
  ...
  static Future<Uint8List?> generateClubbedPdfFromFolder({
    ...
    try {
      ...
      final clubbedPdfBytes = await PdfService.generateClubbedInspectionPdf(inspectionsToMerge);
  ...
  static Future<List<File>> collectPdfsFromPaths(List<String> paths) async {
    ...
    for (final path in paths) {
      ...
      if (entity == FileSystemEntityType.file && path.toLowerCase().endsWith('.pdf')) {
  ...
  static Future<Uint8List?> generateClubbedPdfFromFiles({
    ...
    try {
      ...
      debugPrint('Mapped ${inspectionsToMerge.length} inspections from ${pdfFiles.length} files');
